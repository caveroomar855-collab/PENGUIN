-- Create a named v2 RPC for safer return processing
-- This function implements the same safe logic but has a distinct name
-- so backend can call it even if the original function in the DB wasn't replaced.

CREATE OR REPLACE FUNCTION public.fn_procesar_devolucion_safe_v2(
  p_alquiler uuid,
  p_articulos jsonb,
